        ...
        async function asyncFindUser(list) {
            User.findOne({ email: list.ownerEmail }, (err, user) => {
                ...
                if (user._id == req.userId) {
                    ...
                    console.log("super incorrect user!");
                    return res.status(400).json({ 
        ...
        async function asyncFindUser(list) {
            await User.findOne({ email: list.ownerEmail }, (err, user) => {
                ...
                if (user._id == req.userId) {
                    ...
                    console.log("really incorrect user!");
        ...
        async function asyncFindList(email) {
            ...
            await Playlist.find({$or: [{ ownerEmail: email }, {published: true}]}, (err, playlists) => {
                ...
                if (!playlists) {
                    ...
                    console.log("Send the Playlist");
                   
                    return res.status(200).json({ success: true, data : Object.values(playlists) })
                }
            }).catch(err => console.log(err))
        }
        ...
        async function asyncFindUser(list) {
            await User.findOne({ email: list.ownerEmail }, (err, user) => {
                ...
                if (user._id == req.userId) {
                    ...
                    console.log("wow incorrect user!");
                    return res.status(400).json({ success: false, description: "authentication error" });
                }
            });
        }
